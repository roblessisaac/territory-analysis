...
def build_addresses(row):
    ...
    zip_raw = str(row['Zip_Code'])
    zip_c = zip_raw[:5] if pd.notna(row['Zip_Code']) and zip_raw.lower() != "nan" else ""
...
def generate_excel_report(joined_gdf, kml_gdf, min_goal, max_goal, cong_name):
    ...
    # Excluded Audit logic
    excluded_statuses = ['ROW', 'VACANT LAND', 'PARK'] # Modify if your original list had more
    if 'Status' in joined_gdf.columns:
        excluded_gdf = joined_gdf[joined_gdf['Status'].isin(excluded_statuses)].copy()
        valid_gdf = joined_gdf[~joined_gdf['Status'].isin(excluded_statuses)].copy()
    else:
        excluded_gdf = pd.DataFrame()
        valid_gdf = joined_gdf.copy()
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # --- TAB 3: ADDRESS LIST ---
        # 1. Extract full-precision coordinates from the shapefile's geometry
        ...
        # 3. Add the Suffix, Muni, Latitude, and Longitude to the export
        export_df = address_list_df[['Territory_Name', 'Mailable_Address', 'HouseNo', 'HouseSx', 'Street', 'Unit', 'Muni', 'Zip_Code', 'Latitude', 'Longitude']].rename(columns={'Territory_Name': 'Territory Name', 'Mailable_Address': 'Mailable Address'})
        export_df.to_excel(writer, sheet_name="Address List", index=False)
        
        ws3 = writer.sheets['Address List']
        
        # 4. Hide all columns from C (HouseNo) to J (Longitude)
        for col_letter in ['C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']:
            ws3.column_dimensions[col_letter].hidden = True
            
        # 5. Maintain existing visible column widths
        ...
        pd.DataFrame(columns=["Too Large", "Count", "Too Small", "Count ", "Recommendation"]).to_excel(writer, sheet_name="Border Rewrites", index=False)
        ...
        if len(excluded_gdf) > 0:
            export_ex_df = excluded_gdf[['Territory_Name', 'Mailable_Address', 'Status']].rename(columns={'Territory_Name': 'Territory Name', 'Mailable_Address': 'Mailable Address', 'Status': 'Addr Status'})
            export_ex_df.to_excel(writer, sheet_name="Excluded Audit", index=False)
        else:
            pd.DataFrame(columns=["Territory Name", "Mailable Address", "Addr Status"]).to_excel(writer, sheet_name="Excluded Audit", index=False)
...
# Render Download Button independently to keep it sticky
