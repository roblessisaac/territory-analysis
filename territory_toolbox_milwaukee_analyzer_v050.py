...
APP_VERSION = "TerritoryToolbox Milwaukee Analyzer v0.5.0-no-list-keys"
...
def clean_scalar(value) -> str:
    """Normalize any GIS attribute into a deterministic, hashable string.

    Some KML/GIS drivers can return list-, set-, tuple-, or dict-valued
    attributes. Those values must never enter pandas groupby/sort/merge keys.
    """
    if isinstance(value, dict):
        items = sorted(value.items(), key=lambda item: str(item[0]))
        return "; ".join(
            f"{clean_scalar(key)}={clean_scalar(item_value)}"
            for key, item_value in items
            if clean_scalar(key) or clean_scalar(item_value)
        )

    if isinstance(value, (list, tuple, set, np.ndarray, pd.Series)):
        return "; ".join(
            part for part in (clean_scalar(item) for item in list(value)) if part
        )
...
def clean_text_series(series: pd.Series) -> pd.Series:
    """Normalize every cell to a plain string before pandas hash operations."""
    if series is None:
        return pd.Series(dtype="string")

    return (
        series.map(clean_scalar)
        .astype("string")
...
def sanitize_hash_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Remove all list/dict/set cells from non-geometry columns."""
...
def natural_sort_key(text: object) -> str:
    """Return a hashable natural-sort key; never return a Python list."""
    raw = clean_scalar(text).casefold()
    return "".join(
        part.zfill(12) if part.isdigit() else part
...
def natural_sort_df(df: pd.DataFrame, col: str) -> pd.DataFrame:
    if df.empty or col not in df.columns:
        return df
    result = df.copy()
    result["_sort_key"] = result[col].map(natural_sort_key).astype("string")
    return result.sort_values("_sort_key", kind="mergesort").drop(columns="_sort_key")
...
def load_county_data(county_name: str) -> Optional[gpd.GeoDataFrame]:
    ...
    try:
        try:
            ...
            return sanitize_hash_columns(gpd.read_file(path, engine="pyogrio"))
        except Exception:
            return sanitize_hash_columns(gpd.read_file(path))
...
def load_kml(uploaded_kml) -> gpd.GeoDataFrame:
    ...
    kml_gdf = sanitize_hash_columns(kml_gdf)
...
def assign_addresses_to_territories(
    ...
    address_points = sanitize_hash_columns(address_points)
    territories = sanitize_hash_columns(territories)
    ...
    joined = sanitize_hash_columns(joined)

    # Build assignment QA before deduping conflict rows.
    assigned_rows = joined.dropna(subset=["Territory_Name"])[
        ["Source_Record_ID", "Territory_Name"]
    ].copy()
    assigned_rows["Source_Record_ID"] = clean_text_series(assigned_rows["Source_Record_ID"])
    assigned_rows["Territory_Name"] = clean_text_series(assigned_rows["Territory_Name"])

    assignment_summary = (
        assigned_rows.groupby("Source_Record_ID", dropna=False)
        .agg(
            Matched_Territory_Count=("Territory_Name", "nunique"),
            Matched_Territories=(
                "Territory_Name",
                lambda series: "; ".join(
                    sorted(
                        {clean_scalar(value) for value in series if clean_scalar(value)},
    ...
    joined["_territory_sort"] = joined["Territory_Name"].fillna("ZZZ_UNASSIGNED").map(natural_sort_key).astype("string")
